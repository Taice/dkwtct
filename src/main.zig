...
pub fn main(init: std.process.Init) !void {
    ...
    v.program_start = .now(io, .cpu_process);
    ...
    if (config_file.getKeymapFile(init.io, init.minimal.environ, gpa, default_keymap_str)) |f| {
        if (f) |file_contents| {
    ...
    directory_box.text = std.ArrayList(u8).fromOwnedSlice(
        try config_file.getPreferredSaveDirectory(init.io, gpa, init.minimal.environ),
    );
    ...
    while (!rl.windowShouldClose()) {
        ...
        try keymap.renderForcedAspectRatio(gpa, backend.layout, screen_size, !is_paused);
        ...
        if (is_paused) {
            ...
            _ = try directory_box.render(io, top, "directory", if (is_saving) "SAVE" else "LOAD");
            _ = try layout_box.render(io, middle, "layout", "");
            if (try variant_box.render(io, bottom, "variant", "") and layout_box.text.items.len != 0 and variant_box.text.items.len != 0) {
                const layout = root.trim(layout_box.text.items);
                const variant = root.trim(variant_box.text.items);

                if (is_saving) {
                    backend.saveLayoutNameVariant(io, gpa, layout, variant) catch |e| {
                        std.debug.print("Error while saving layout: {any}\n", .{e});
                    };
                } else b: {
                    backend.importLayoutNameVariant(io, gpa, layout, variant) catch |e| {
                        std.debug.print("Error while importing layout: {any}\n", .{e});
                        break :b;
                    };
                    try rlf.addLayoutToFont(gpa, backend.layout, &v.char_font);
                }
                is_paused = false;
                is_saving = false;
            }
            ...
            if (keybind("ctrl+v")) c: {
                ...
                if (v.selected_button) |button| b: {
                    ...
                    backend.layout.* = ly;
                    try rlf.addLayoutToFont(gpa, &ly, &v.char_font);
                }
            }
            if (keybind("ctrl+n")) {
            ...
            if (keybind("ctrl+s")) {
                if (rl.isKeyDown(.left_shift)) {
                    ...
                    if (!try backend.trySavingLayout(io, gpa)) {
                        is_paused = true;
                        is_saving = true;
                    }
                }
    ...
    try config_file.saveSaveDirectory(io, gpa, init.minimal.environ, directory_box.text.items);
}
