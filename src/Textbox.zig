...
pub fn init(gpa: Allocator) Textbox {
    return .{
        .gpa = gpa,
    };
}

pub fn render(ts: *Textbox, io: Io, bounds: rl.Rectangle, no_text: [:0]const u8, top_text: [:0]const u8) !bool {
    ...
    if (ts.selected) {
        if (@rem(@divTrunc(v.program_start.untilNow(io, .real).nanoseconds, (std.time.ns_per_s / 2)), 2) == 0)
