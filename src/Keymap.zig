...
const Element = union(enum) {
    ...
    pub fn parse(str: []const u8, gpa: Allocator) !Element {
        // validation check
        for (str) |char| {
            switch (char) {
                'a'...'z',
...
pub fn renderToFill(ts: *Keymap, gpa: Allocator, layout: *Layout, dims: rl.Rectangle, focused: bool) !void {
    const gap_size = 0;
    ...
    if (rl.isWindowResized()) {
        ...
        if (v.char_font.baseSize != new_size) {
            try rlf.resizeFont(gpa, &v.char_font, new_size);
        }
    }
...
pub fn renderForcedAspectRatio(ts: *Keymap, gpa: Allocator, layout: *Layout, dims: rl.Rectangle, focused: bool) !void {
    ...
    try ts.renderToFill(gpa, layout, .init(base_coords.x, base_coords.y, size.x, size.y), focused);
}
